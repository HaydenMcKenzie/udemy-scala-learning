...
object HOFsCurries extends App {
  ...
  // returns is anything after the => with () around it

  // higher order function
  // Examples of what I have done with HOFs: Map, FlapMap and Filter from MyList.Scala

  // function that applies a function n times over a value x
  // nTimes(f, n, x)
  // nTimes(f, 3, x) == f(f(f(x))) or nTimes(f, 2, f(x)) === f(f(f(x)))
